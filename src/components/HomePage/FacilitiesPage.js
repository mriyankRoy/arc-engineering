...
export default function FacilitiesPage() {
  ...
  return (
    <section className="bg-[#fcfcfc] py-24 px-6 md:px-20 text-[#44444E]" aria-labelledby="facilities-title">
      <Helmet>
      ...
      <header className="max-w-[1400px] mx-auto mb-24">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
          <h3 id="facilities-title" className="text-3xl md:text-5xl font-semibold leading-tight uppercase tracking-tight">
          ...
          <p className="max-w-md text-[#44444E]/50 font-bold uppercase tracking-widest text-[10px] leading-relaxed pb-1 border-l-2 border-[#44444E]/10 pl-6 lg:ml-8">
            Precision <strong>manufacturing</strong> via ISO-certified high-capacity terminals and <strong>industrial fabrication</strong> hubs.
          </p>
        </div>
      </header>
        ...
        {facilities.map((f, i) => (
          <div key={f.id} className="snap-center h-auto">
            <div className="w-full aspect-square rounded-[2rem] overflow-hidden mb-6 bg-black shadow-xl relative">
              ...
              />
              <div className="absolute inset-0 rounded-[2rem] border-[1px] border-white/10 pointer-events-none" aria-hidden="true" />
            </div>
      ...
      <div className="hidden lg:flex max-w-[1400px] mx-auto gap-32 relative">
        <aside className="w-1/2 sticky top-[calc(50vh-300px)] h-[600px] z-10" aria-hidden="true">
...
function Section({ data, index, onActive, isMobile }) {
  ...
  return (
    <article ref={ref} className={`relative mb-12 ${isMobile ? "pl-0" : "pl-12 min-h-[80vh]"}`}>
      <div className={`absolute left-0 top-0 bottom-0 w-[2px] bg-[#44444E]/10 ${isMobile ? "hidden" : "block"}`} aria-hidden="true">
      ...
      <div className="bg-white p-8 rounded-3xl border border-[#eeeeee] shadow-[0_10px_30px_-5px_rgba(0,0,0,0.05)]">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-[#f8f8f8] p-4 rounded-2xl border border-[#f0f0f0]">
            <span className="block text-[8px] font-bold text-[#44444E]/40 uppercase mb-1">Production Capacity</span>
        ...
        <ul className="space-y-3 mb-8" aria-label="Facility Highlights">
          {data.highlights?.slice(0, 3).map((text, idx) => (
            <li key={idx} className="flex items-center gap-3 text-[#44444E]/60">
              <CheckCircle2 size={14} className="text-[#BF092F] flex-shrink-0" aria-hidden="true" />
        ...
        >
          Explore Unit <ArrowRight size={16} className="group-hover:translate-x-2 transition-transform duration-300" aria-hidden="true" />
        </Link>
      </div>
    </article>
  );
