...
const Footer = () => {
  ...
  return (
    <footer className="relative w-full px-2 pb-2 mt-12 overflow-hidden" role="contentinfo">
      ...
      <div className="relative w-full mx-auto rounded-2xl shadow-2xl overflow-hidden bg-[#44444E]">
        ...
        <div className="container mx-auto px-6 md:px-10 lg:px-12 py-16 lg:py-20 relative z-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-x-8 gap-y-12 items-start">
            ...
            <div className="lg:col-span-4 flex flex-col items-start space-y-6 md:space-y-8">
              ...
              <p className="text-white/70 text-[10px] md:text-[11px] font-bold uppercase tracking-[0.25em] leading-loose max-w-sm border-l-2 border-[#BF092F] pl-5 italic">
                Engineering excellence in power solutions. Supporting UK, Europe, and Worldwide markets with bespoke industrial reliability.
              </p>
              <nav className="flex gap-3" aria-label="Social Media">
                {[Facebook, Twitter, Linkedin].map((Icon, i) => (
                  <a key={i} href="#" className="p-3 bg-white/5 border border-white/10 text-white hover:bg-white hover:text-[#BF092F] transition-all rounded-xl">
                    <Icon size={16} />
                  </a>
                ))}
              </nav>
            ...
            <nav className="lg:col-span-2">
              <h3 className="text-[10px] font-black text-white/30 uppercase tracking-[0.5em] mb-8 lg:mb-10">Directory</h3>
                  ...
                  <li key={link.name}>
                    <Link 
                      ...
                      className={`text-[11px] font-bold uppercase tracking-[0.2em] flex items-center gap-2 group transition-all ${
                        location.pathname === link.path ? "text-[#BF092F]" : "text-white hover:text-white/60"
                      }`}
                    >
                      <ArrowRight size={12} className={`transition-all ${location.pathname === link.path ? "opacity-100 ml-0" : "opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-0"} text-[#BF092F]`} />
                      {link.name}
                    </Link>
                  </li>
                ))}
            ...
            <nav className="lg:col-span-3">
              <h3 className="text-[10px] font-black text-white/30 uppercase tracking-[0.5em] mb-8 lg:mb-10">Product_Scope</h3>
              <ul className="space-y-4 md:space-y-5">
            ...
            <div className="lg:col-span-3">
              <h3 className="text-[10px] font-black text-white/30 uppercase tracking-[0.5em] mb-8 lg:mb-10">Contact_Terminal</h3>
              <div className="space-y-6 md:space-y-7">
                ...
                <div className="flex items-center gap-4 group">
                  ...
                  <a href="mailto:info@arcengltd.com" className="text-[11px] font-bold text-white uppercase tracking-widest transition-colors group-hover:text-white break-all">
                    info@arcengltd.com
                  </a>
                </div>
                <div className="flex items-center gap-4 group">
                  <Phone className="text-white/40 group-hover:text-white transition-colors shrink-0" size={16} />
                  <a href="tel:+447492949230" className="text-[11px] font-bold text-white uppercase tracking-widest transition-colors group-hover:text-white">
                    +44 7492 949230
                  </a>
                </div>
              </div>
          ...
          <div className="mt-16 md:mt-20 pt-8 md:pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8">
            <p className="text-[9px] font-bold text-white/20 uppercase tracking-[0.4em] text-center md:text-left">
              &copy; {currentYear} ARC ENGINEERING SOLUTIONS LTD <span className="mx-2 text-[#BF092F]">/</span> ALL_RIGHTS_RESERVED
            </p>
        ...
        {/* Visual Watermark */}
        <span className="absolute bottom-[-10px] md:bottom-[-15px] right-[-10px] text-[80px] md:text-[180px] font-black text-white/[0.02] select-none pointer-events-none tracking-tighter leading-none" aria-hidden="true">
          ARC
        </span>
      </div>
    </footer>
