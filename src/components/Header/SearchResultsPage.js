...
import { Search, Home, ChevronLeft, ChevronRight, PackageSearch } from "lucide-react";
        ...
        <header className="shadow-xl relative h-[28vh] min-h-[300px] w-full flex items-center bg-[#44444E] overflow-hidden rounded-2xl">
          ...
          <div className="container mx-auto px-4 md:px-6 relative z-20">
            <nav className="flex items-center flex-wrap gap-3 mb-6">
              ...
              <button onClick={() => navigate("/products")} className="text-[10px] md:text-xs tracking-widest uppercase text-white/50 hover:text-white transition-colors">Product Categories</button>
              <span className="text-white/20 text-xs font-mono">{">"}</span>
              <button className="text-[10px] md:text-xs tracking-widest uppercase bg-[#BF092F] text-white px-4 py-1.5 rounded-2xl shadow-lg shadow-[#BF092F]/20 font-bold">Search Results</button>
            </nav>
            <div>
        ...
        <div className="bg-white p-8 md:p-12 rounded-2xl shadow-xl border border-gray-100 min-h-[600px] flex flex-col">
          ...
          {results.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="text-xl font-bold text-[#44444E] mb-2 uppercase tracking-tight">No Records Found</h3>
              <button onClick={() => navigate("/products")} className="flex items-center gap-3 px-8 py-4 bg-[#44444E] text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-xl hover:bg-[#BF092F] transition-all shadow-lg">
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 flex-grow">
                {paginatedResults.map((item, idx) => (
                  <div key={item.id || idx} className="transition-all duration-500 hover:-translate-y-2">
